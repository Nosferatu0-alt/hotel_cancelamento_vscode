...
1. **Carregamento e amostragem** — amostra estratificada de 7.500 registros para balancear as classes
...
6. **Validação Cruzada** — Stratified K-Fold (k=5) para avaliação robusta
...
| **Gradient Boosting** | **80,80%** | **81,60%** | **62,23%** | **70,61%** | **0,8661** |
...
- **Lead time** — tempo entre a reserva e o check-in
- **Tipo de depósito** — se o depósito é reembolsável ou não
- **Segmento de mercado** — canal pelo qual a reserva foi feita
- **Histórico de cancelamentos** — cancelamentos anteriores do cliente
- **Tipo de cliente** — individual ou grupo
...
- A amostragem de 10.000 registros foi necessária por questões de desempenho, representando cerca de 8,4% do dataset completo (~119.000 registros), o que pode reduzir a capacidade do modelo de capturar padrões menos frequentes
...
O projeto demonstrou que é possível prever cancelamentos de reservas hoteleiras com boa acurácia utilizando técnicas de Machine Learning. O **Gradient Boosting** se destacou como o modelo mais adequado, atingindo **80,8% de acurácia** e **AUC-ROC de 0,866**, mostrando boa capacidade de discriminação entre reservas mantidas e canceladas.
